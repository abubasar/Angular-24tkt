...
currentJustify = 'center';
  ...
  ngOnInit() {
    this.state$ = this.router.events.pipe(
      ...
      map((x:NavigationStart) => {
        const currentNav = this.router.getCurrentNavigation().extras.state={...x.restoredState, navigationId:x.id };
        return currentNav;
      })
    );
  ...
  searchFlight(){
    ...
    this.flightService.getSearchKey(this.searchRequest).subscribe(res=>{
      ...
      this.flightService.searchFlights(this.searchRequest).subscribe(res=>{
         this.router.navigateByUrl('/search', {state: {data: {res}}});
